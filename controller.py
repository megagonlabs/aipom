...
class Controller:
    ...
    def process_user_message(
        ...
        if action["action"] == 1:  # plan
            ...
            plan_dag = None
            system_message = self._generate_response(
                action, response_to=user_message["id"]
            )
    ...
    def process_ui_interaction(
        self, interaction: InteractionData, response_to: int = -1
    ) -> tuple[MultiDiGraph | None, SystemMessage | None]:
        """
        ...
        if not prev_plan:
            return None, self._generate_response(
                action={"action": 5, "ex": "No plan to update"}, response_to=response_to
            )
    ...
    def process_execution(
        self, exec_request: ExecuteData, response_to: int = -1
    ) -> tuple[MultiDiGraph | None, SystemMessage | None]:
        """
        ...
        if mode == "all":
            ...
            if not self.executor.plan_dag and not self.executor.is_source_node(node_id):
                return None, self._generate_response(
                    action={
                        "action": 5,
                        "ex": "Cannot execute node without executing previous nodes",
                    },
                    response_to=response_to,
                )
            ...
            if not self.executor.can_execute_node(node_id):
                return None, self._generate_response(
                    action={
                        "action": 5,
                        "ex": "This node cannot be executed yet because one or more of its preceding nodes have not been executed. Please make sure all prerequisite nodes are completed before proceeding.",
                    },
                    response_to=response_to,
                )
            ...
            try:
                ...
                return self.executor.get_plan().dag, self._generate_response(
                    action={
                        "action": 5,
                        "ex": f"Error executing node {node_id}. Ensure edges are connected, i/o variables defined.",
                    },
                    response_to=response_to,
                )
            plan = self.executor.get_plan()
            system_response = self._generate_response(
                {"action": 3, "execute": {"mode": "single", "node_id": node_id}},
                response_to=response_to,
                plan=plan,
            )
    ...
    # def load(self, query: str, dag: MultiDiGraph, chat_history: list) -> tuple[MultiDiGraph | None, SystemMessage | None, list[Message]]:
    #     """Resets controller state"""
    #     self.interaction_log.clear()
    #     self.chat_history = chat_history
    #     dag.graph["query"] = query
    #     plan = PlanDAG().initialize_from_dag(dag)
    #     self.planner.plan_history = [plan]
    #     self.executor = Executor(self.registry)
    #     return plan.dag, self.chat_history[-1], self.chat_history
    ...
    def _classify_intent(self) -> Action:
        ...
        try:
            ...
            print(
                f"[{current_time()}] -- Error in intent detection:",
                e,
    ...
    def _generate_response(
        self, action: Action, response_to: int, plan: str = ""
    ) -> SystemMessage:
        """
        ...
        if action["action"] == 0:
            ...
            if action["action"] == 1:
                # plan
                prompt = RESPONSE_MESSAGE_PLAN.format(
                    query=action["user_query"], plan=plan
                )
            elif action["action"] == 2:
                # feedback
                prompt = RESPONSE_MESSAGE_PLAN.format(
                    query=action["plan_feedback"], plan=plan
                )
            ...
            try:
                ...
                print(
                    f"[{current_time()}] -- Error in response generation:",
                    e,
                    traceback.format_exc(),
                )
