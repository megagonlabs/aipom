...
from prompts import PLAN_FIX_PROMPT, PLAN_REFINE_PROMPT, PLAN_SYSTEM_PROMPT
...
class Planner:
    ...
    def __init__(self, agent_registry):
        ...
        self.plan_history: list[PlanDAG] = []
    ...
    def refine_plan_nl(self, feedback: str) -> PlanDAG:
        ...
        plan = PlanDAG().initialize_from_LLMPlan(
            prev_plan.query, llm_plan, self.agent_names
        )
        plan.initialize_params(agent_registry=self.agent_registry)
        self.plan_history.append(plan)
        return plan
    ...
    def _llm_fixer(self, query, plan):
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": self.fix_plan_prompt.format(query=query, plan=plan),
            },
        ]
        response = self.client.beta.chat.completions.parse(
            messages=messages, **self.config
