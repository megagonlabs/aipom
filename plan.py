...
class PlanDAG:
    ...
    def initialize_from_dag(self, dag: MultiDiGraph) -> "PlanDAG":
        ...
        dag.graph.setdefault("id", create_uuid())
        dag.graph.setdefault("timestamp", current_time())
        self.dag = dag
    ...
    def initialize_from_LLMPlan(
        ...
        self.query = query
        self.set_plan_status("PLANNED")
        self.set_exec_status("NONE")
        return self

    # def initialize_from_UIPlan(self, plan: UIPlan) -> "PlanDAG":
    #     """Initializes the DAG from a UI plan."""
    #     self.dag = PlanConverter.dag_from_UIPlan(plan, agent_names)
    ...
    def update_node(self, node_id, node_data):
        ...
        self.set_node_plan_status(node_id, "MODIFIED")
    ...
    def update_node_edge(self, node_id, node_data, edges):
        ...
        for edge in edges:
            ...
            edge["data"]["plan_status"] = "MODIFIED"
    ...
    def update_exec(self, node_id, node_exec, node_attr, node_attr_value):
        ...
        try:
            for _, _, k, d in self.dag.out_edges(node_id, data=True, keys=True):
                if k[0] == node_attr:
                    if node_data["exec"][k[0]] == node_attr_value:
                        d["exec_in_sync"] = True
                    else:
        ...
        self.dag.nodes[node_id].update(node_data)
        self.set_node_exec_status(node_id, "MODIFIED")
        ...
        edges = [
            {
                ...
                "animated": e.get("exec_in_sync", False),
                "data": {
                    **e,
                },
            }
