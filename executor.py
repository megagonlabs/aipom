...
class Executor:
    ...
    def can_execute_node(self, node_id):
        ...
        return all(
            self.plan_dag.nodes[pred]["exec_status"] in valid_status
    ...
    def execute_node(self, node_id):
        ...
        try:
            for src_id, _, d in self.plan_dag.in_edges(node_id, data=True):
                input_vals[d["dest_input"]] = self.plan_dag.nodes[src_id]["exec"][
                    d["src_output"]
                ]
            ...
            raise Exception(
                f"Error executing node {node_id}: Ensure edges are connected, i/o variables defined.    "
            )

        try:
            self.plan_dag.nodes[node_id]["exec"] = agent.execute(
                task, input_vars, output_vars, params
            )
            self.plan_dag.nodes[node_id]["exec_status"] = "EXECUTED"
        except Exception as ex:
            raise Exception(
                f"Error executing node {node_id}: Ensure edges are connected, i/o variables defined."
            )
        ...
        try:
            for _, dest_id, k, d in self.plan_dag.out_edges(
                node_id, data=True, keys=True
            ):
                if (
                    original_exec
                    and original_exec[k[0]]
                    == self.plan_dag.nodes[node_id]["exec"][k[0]]
                ):
                    d["exec_in_sync"] = True
                else:
                    d["exec_in_sync"] = False
        except Exception as ex:
            raise Exception(ex)
